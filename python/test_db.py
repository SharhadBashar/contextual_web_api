    ...
    def write(self):
        ...
        INSERT INTO 
                   ...
                   VALUES 
                    ...
                    '{}', '{}', 'test description')
        """.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), datetime.now().strftime('%Y-%m-%d %H:%M:%S'), str(topics), str(topics_match))
        cursor = conn.cursor()
        cursor.execute(query)
        conn.commit()
        cursor.close()
